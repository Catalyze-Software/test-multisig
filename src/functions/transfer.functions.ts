...
export function transferTest() {
  ...
  it("Should send identity one DIP20 funds to multisig wallet", async () => {
    ...
    let result = await dip20One.transfer(Principal.fromText(multisig_canister_id), BigInt(1000000000));
    expect(result).toEqual({ Ok: BigInt(0) });
  });

  it("Multisig should have balance of 1000000000 DIP20", async () => {
    let result = await dip20One.balanceOf(Principal.fromText(multisig_canister_id));
    console.log(result);
  ...
  it("Multisig should have balance of 1000000000 ICRC", async () => {
    let result = await icrcOne.icrc1_balance_of({ owner: Principal.fromText(multisig_canister_id), subaccount: [] });
    console.log(result);
    expect(result).toBeGreaterThanOrEqual(BigInt(1000000000));
  });

  it("Should add identity three to the whitelist", async () => {
    const balance = await dip20One.balanceOf(Principal.fromText(multisig_canister_id));
  ...
  it("identity one should create a transfer request of 100_000_000 DIP20 tokens to identity two", async () => {
    let result = await msOne.transaction_request(Principal.fromText(dip20_canister_id), {
      DIP20: {
  ...
  it("Should approve the transaction request", async () => {
    let result = await msThree.vote_on_transaction_request(0, { Approve: null });
  ...
  it("identity one should create a transfer request of 100_000_000 ICRC1 tokens to identity two", async () => {
    let result = await msOne.transaction_request(Principal.fromText(icrc1_canister_id), {
      ICRC1: {
        amount: BigInt(100000000),
        to: {
  ...
  //
  it("Should approve the transaction request", async () => {
    let result = await msThree.vote_on_transaction_request(1, { Approve: null });
  ...
  it("Identity two should have a balance of 100000000 ICRC1 tokens", async () => {
    let result = await icrcOne.icrc1_balance_of({ owner: identities.two().getPrincipal(), subaccount: [] });
    expect(result).toEqual(BigInt(100000000));
  });
